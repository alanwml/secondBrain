...
async function callProvider(thought) {
  ...
  const response = await fetch(`${BASE_URL}/responses`, { method:'POST', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${apiKey}` }, body:JSON.stringify({ model, tools:[{ type:'web_search_preview' }], include:['web_search_call.action.sources'], store:false, input:[{ role:'developer', content:'You are a thoughtful research assistant for a personal second brain. Analyze the user thought without changing its intent. Use web search when useful. Return ONLY valid JSON with keys: summary (string), explanation (string), keyPoints (array of strings). Do not invent citations; source links are collected separately from the web search tool.' }, { role:'user', content:`Analyze this ${thought.type}:\n\n${thought.text}` }] }) });
  ...
  return { ...parsed, sources, images, model:data.model || model, inputTokens:data.usage?.input_tokens ?? null, outputTokens:data.usage?.output_tokens ?? null, costUsd:priceFor(data.model || model, data.usage), pricing:PRICING[data.model || model] ? { inputPerMillion:PRICING[data.model || model].input, outputPerMillion:PRICING[data.model || model].output } : null, completedAt:new Date().toISOString() };
}
async function processJob(job) { const thought = findThought(job.thoughtId); if (!thought) { job.status = 'failed'; job.error = 'Thought not found'; return; } job.status = 'processing'; job.startedAt = new Date().toISOString(); job.attempts += 1; thought.aiStatus = 'processing'; thought.aiError = null; writeJson(JOBS_FILE, jobs); writeJson(THOUGHTS_FILE, thoughts); try { thought.aiAnalysis = await callProvider(thought); thought.aiStatus = 'completed'; job.status = 'completed'; job.completedAt = new Date().toISOString(); } catch (error) { thought.aiStatus = 'failed'; thought.aiError = error.message; job.status = 'failed'; job.error = error.message; job.completedAt = new Date().toISOString(); } writeJson(JOBS_FILE, jobs); writeJson(THOUGHTS_FILE, thoughts); }
